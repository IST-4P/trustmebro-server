...
async function bootstrap() {
  ...
  const port = AppConfiguration.ORDER_SERVICE_PORT || 3008;

  app.connectMicroservice(GrpcServerOptions(GrpcService.ORDER_SERVICE));
  app.connectMicroservice(KafkaServerOptions(QueueGroups.ORDER_GROUP));
