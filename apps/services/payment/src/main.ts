...
async function bootstrap() {
  ...
  const port = AppConfiguration.PAYMENT_SERVICE_PORT || 3009;

  app.connectMicroservice(GrpcServerOptions(GrpcService.PAYMENT_SERVICE));
  app.connectMicroservice(KafkaServerOptions(QueueGroups.PAYMENT_GROUP));
